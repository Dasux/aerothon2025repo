...
3S)} & 360  \\
  ...
  % ======== Aircraft Performance ====
    \section{Aircraft Performance}
      ...
      \end{table}
        ...
        The avionics system forms the central nervous system of any unmanned aerial vehicle (UAV), including those designed for Aerothon-class missions. Its design and power requirements significantly influence the overall electrical and power architecture of the aircraft. The avionics suite typically comprises a flight controller (e.g., Pixhawk, ArduPilot), GPS module, telemetry transceiver, power module, airspeed sensor, magnetometer, and optional payload components such as cameras or LIDAR.\\

        For our configuration, we estimate a continuous power draw of approximately 4--7~W for the flight control unit, with peak consumption occurring during sensor fusion and control loop calculations. GPS and telemetry systems combined account for an additional 2--3~W, while payload elements such as a live camera feed (ESP32-CAM or similar) can add up to 2~W intermittently. These figures necessitate a robust power supply strategy, typically sourced from a regulated DC bus connected to the main propulsion battery, with onboard voltage regulation via buck converters to ensure clean and stable 5~V and 3.3~V rails. \\

        Additionally, electromagnetic interference (EMI) shielding and power line filtering are considered critical to maintain reliable communication and sensor accuracy during flight. The avionics bay is thermally isolated and strategically placed near the center of gravity to reduce vibration and ensure precise attitude estimation.\\

        Overall, the avionics demand, while modest in terms of raw power, is mission-critical and demands high reliability, low latency, and efficient power distribution to support stable flight control and telemetry feedback loops.
  ...
  %========= Ground Control
    \section{Avionics Subsystems Selection}
      \subsection{Flight Controller, Flight Computer, Cameras}
    ...
    \subsubsection{\large Avionics Powertrain Calculations}
